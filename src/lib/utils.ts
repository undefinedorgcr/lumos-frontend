...
export function findActiveTick(price: number, liquidityData: any[], t0: Token, t1: Token) : number {
  let returnTick = 0;
  [...liquidityData].forEach(({ tick }: { tick: number }) => {
    const tickPrice = tickToPrice(Number(tick)) * (10 ** (t0.decimals - t1.decimals));
    if (tickPrice <= price) {
      returnTick = tick;
    }
    else {
...
export function getTicksInRange(minTick: number, maxTick: number, liquidityData: any[], tick_spacing: number) {
  const ticks: number[] = [];
  [...liquidityData].forEach(({ tick }: { tick: number }) => {
...
export function getTickFromPrice(price: number, t0: Token, t1: Token, liquidityData: any[], tick_spacing: number) {

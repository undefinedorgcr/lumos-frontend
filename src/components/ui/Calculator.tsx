...
const Calculator: React.FC<CalculatorProps> = ({ token0, token1, feeRate, initialPrice, volume }) => {
    ...
    async function handlePricesChange(min: number, max: number, amount: number) {
        const currentLiquidity = await fetchLiquidityInRange(minPrice, maxPrice, liquidityData);
                                    ...
                                    <div key={token.symbol} className="bg-zinc-900 rounded-lg p-3 flex items-center justify-between">
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm overflow-x-auto whitespace-nowrap min-w-16 max-w-32">
                                                {(depositAmounts[index] / (index === 0 ? t0CurrentPrice : t1CurrentPrice)).toFixed(4)}
                                            </p>
                                            <p className="text-xs text-zinc-400 overflow-x-auto whitespace-nowrap">
                                                ${depositAmounts[index].toFixed(2)}
                                            </p>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="bg-zinc-800/50 rounded-lg p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-zinc-800 rounded-lg p-3">
                                    ...
                                    <p className="text-lg font-medium overflow-x-auto whitespace-nowrap">{minPrice.toFixed(6)}</p>
                                ...
                                <div className="bg-zinc-800 rounded-lg p-3">
                                    ...
                                    <p className="text-lg font-medium overflow-x-auto whitespace-nowrap">{maxPrice.toFixed(6)}</p>
                                    <span className="text-xs text-zinc-400">{token1.symbol} per {token0.symbol}</span>
                                </div>
                            </div>
                        </div>

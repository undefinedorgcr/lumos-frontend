  ...
  useEffect(() => {
    async function setupCalculator() {
      try {
        ...
        setLiquidityData(
          await fetchLiquidityData(token0, token1, minPrice, maxPrice, feeRate)
        );
        setIsLoading(false);
      } catch (err) {
        console.log(err);
  ...
  async function handlePricesChange(min: number, max: number, amount: number) {
    ...
    if (volume !== null && currentLiquidity != null) {
      ...
      const liquidityAmount0 =
        (amounts[0] * 10 ** token0.decimals * (Math.sqrt(Pu) * Math.sqrt(Pl))) /
        (Math.sqrt(Pu) - Math.sqrt(Pl));
      const liquidityAmount1 =
        (amounts[1] * 10 ** token0.decimals) / (Math.sqrt(Pu) - Math.sqrt(Pl));
      const deltaL = Math.min(liquidityAmount0, liquidityAmount1);
      const fee =
        (feeRate / 100) * volume * (deltaL / (currentLiquidity + deltaL));
      setFee(fee / 100);
    }
  }
                    ...
                    <div className="text-right">
                      <p className="text-sm overflow-x-auto whitespace-nowrap min-w-16 max-w-32">
                        {(
                          ...
                          (index === 0 ? t0CurrentPrice : t1CurrentPrice)
                        ).toFixed(4)}
                      </p>

...
export default function LiquidityChart({
    ...
    const baseData = liquidityData.length > 0
    ...
    const maxLiquidity = Math.max(...formattedData.map(item => Math.abs(item.liquidity)));
    ...
    const normalizedData = formattedData.map(item => {
        ...
        const normalizedValue = Math.sqrt(absLiquidity) / (maxLiquidity > 0 ? Math.sqrt(maxLiquidity) : 1);

        return {
            ...item,
            normalizedLiquidity: liquidityValue >= 0 ? normalizedValue : -normalizedValue,
            ...
            <div className="flex flex-col h-full p-4">
                ...
                <div className="grid grid-cols-3 gap-4 mt-4">
                    <div className="bg-zinc-800 rounded p-2">
                        ...
                        <p className="text-sm font-medium">{typeof validMinPrice === 'number' ? validMinPrice.toFixed(4) : validMinPrice}</p>
                        <p className="text-xs text-zinc-500">{token1.symbol} per {token0.symbol}</p>
                    </div>
                    <div className="bg-zinc-800 rounded p-2 border border-pink-500/30">
                        <p className="text-xs text-zinc-400">Current Price</p>
                        <p className="text-sm font-medium text-pink-400">{typeof validCurrentPrice === 'number' ? validCurrentPrice.toFixed(4) : validCurrentPrice}</p>
                        <p className="text-xs text-zinc-500">{token1.symbol} per {token0.symbol}</p>
                    </div>
                    <div className="bg-zinc-800 rounded p-2">
                        <p className="text-xs text-zinc-400">Max Price</p>
                        <p className="text-sm font-medium">{typeof validMaxPrice === 'number' ? validMaxPrice.toFixed(4) : validMaxPrice}</p>
                        <p className="text-xs text-zinc-500">{token1.symbol} per {token0.symbol}</p>
                    </div>
                </div>
            </div>
